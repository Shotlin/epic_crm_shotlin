...
| P1-01 | P1 | Retail Core UAT | CERTIFICATION_REQUIRED | The current UI acceptance catalog contains 51 role-based journeys across cashier, store-manager, HQ/finance, and administrator personas; packaged automation covers the retail rail, POS restart, offline recovery/conflict recovery, intelligence, maintenance, and owner restart. Critical cashier/manager journeys still lack independent current-build evidence. | Retail operations | Signed cashier + store-manager evidence for the certification pack. |
